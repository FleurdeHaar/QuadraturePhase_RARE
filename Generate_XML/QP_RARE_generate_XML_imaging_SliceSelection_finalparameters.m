...
for i = 1:n_prep
    ...
    fprintf(fid, '            <ATOMICSEQUENCE Name="RFprep%d">\n', i);
    % Crusher 1
    ...
    % Crusher 2 
    fprintf(fid, '               <TRAPGRADPULSE Area="2*Pi/(Dz/2)" Axis="GZ" InitialDelay="FTT+CR1" Name="Crush2%d" Observe="Dz=P.Dz, CR1=Crush1%d.Duration,FTT=SS2%d.EndOfFlatTop"/>\n', i, i, i);
    ...
    fprintf(fid, '            <DELAYATOMICSEQUENCE Delay="TE-CR1" DelayType="B2E" Name="D2%d" Observe="TE=P.TE, CR1=Crush1%d.Duration" StartSeq="RFprep%d"/>\n', i,i,i);
...
for pe = 1:((Ny/2)+((n_img-Ny)/2))
    for rep = 1:2   % two echoes per PE step
        ...
        % refocussing pulse + slice selection + crushers 
        % RF
        fprintf(fid, '         <ATOMICSEQUENCE Name="RFimg%d">\n', i);
        fprintf(fid, '            <TRAPGRADPULSE Area="2*Pi/(Dz/2)" Axis="GZ" InitialDelay="RUT" Name="Crush1img%d" Observe="Dz=P.Dz, RUT=SSimg%d.RampUpTime"/>\n', i, i);
        fprintf(fid, '            <SINCRFPULSE Apodization="0.5" Bandwidth="8" FlipAngle="%.10g" InitialDelay="CR1" InitialPhase="%.10g" Name="PimgRF%d" Refocusing="1" Zeros="4" Observe="CR1=Crush1img%d.Duration"/>\n', flip_angle, rf_phase,i, i);
        fprintf(fid, '            <TRAPGRADPULSE Axis="GZ" FlatTopArea="2*Pi/FOVz" FlatTopTime="D" InitialDelay="CR1" Name="SSimg%d" Observe="D=PimgRF%d.Duration, FOVz=P.FOVz, CR1=Crush1img%d.Duration"/>\n', i, i, i);
        fprintf(fid, '            <TRAPGRADPULSE Area="2*Pi/(Dz/2)" Axis="GZ" InitialDelay="FTT+CR1" Name="Crush2img%d" Observe="Dz=P.Dz, CR1=Crush1img%d.Duration, FTT=SSimg%d.EndOfFlatTop"/>\n', i, i, i);
        fprintf(fid, '         </ATOMICSEQUENCE>\n');
        ...
        % Delay of TE, minus the crusher duration
        fprintf(fid, '            <DELAYATOMICSEQUENCE Delay="TE-CR1" DelayType="B2E" Name="D3%d" Observe="TE=P.TE, CR1=Crush1img%d.Duration" StartSeq="RFimg%d"/>\n', i, i,i);
