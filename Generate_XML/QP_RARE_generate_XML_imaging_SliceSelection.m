...
% Settings
name = 'RARE_quadphase_schemefunction_180_7pre_scanner_SS.xml' ;% Set name for file saving
...
fprintf(fid, '            <TRAPGRADPULSE Axis="GZ" Area="-3.14" Name="SS_Deph1"/>\n');
fprintf(fid, '            <HARDRFPULSE Axis="RF" Duration="0.1" FlipAngle="90" InitialPhase="%.10g" Name="P1"/>\n', excitation_initialphase);
fprintf(fid, '            <TRAPGRADPULSE Axis="GZ" Area="3.14" Name="SS_Reph1"/>\n');
...
for i = 1:n_prep
    ...
    %fprintf(fid, '            <DELAYATOMICSEQUENCE Delay="TE" DelayType="C2C" Name="D%d" Observe="TE=P.TE" StartSeq="RFPprep%d" StopSeq=" %d"/>\n', i-1,i);
